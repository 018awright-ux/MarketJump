...
async function splitVideo(
  ...
  if (duration <= SEGMENT_DURATION) {
    ...
    return [{ blob: file, url, duration, name: file.name }]
  }
  ...
  ff.on('progress', ({ progress }) => {
    onProgress(`Splitting video... ${Math.round(progress * 100)}%`)
  })
  ...
  for (let i = 0; i < Math.min(segmentCount, MAX_CLIPS); i++) {
    ...
    // Copy into a regular ArrayBuffer (ffmpeg may return SharedArrayBuffer)
    const raw = data instanceof Uint8Array ? data : new TextEncoder().encode(data as string)
    const copy = new Uint8Array(raw.length)
    copy.set(raw)
    const blob = new Blob([copy], { type: 'video/mp4' })
    const url = URL.createObjectURL(blob)
    const clipDuration = Math.min(SEGMENT_DURATION, duration - start)
    clips.push({ blob, url, duration: clipDuration, name: `clip_${i + 1}.mp4` })
...
export default function CreatePost({ onClose, onPosted }: CreatePostProps) {
  ...
  async function handlePost() {
    ...
    if (!clips.length) { setError('Add at least one video clip.'); return }
    ...
    try {
      ...
      for (let i = 0; i < clips.length; i++) {
        const clip = clips[i]
        const path = `${user.id}/${tempId}/${i}.mp4`

        const { error: uploadError } = await supabase.storage
          .from('post-videos')
          .upload(path, clip.blob, { contentType: 'video/mp4', upsert: false })
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'rgba(4,7,13,0.97)' }}>
      {/* Header */}
      <div className="flex items-center justify-between px-5 pt-6 pb-4 border-b border-[#1e2d4a]">
        ...
        <button
          onClick={step === 'clips'
            ? () => { if (clips.length && !busy) setStep('details') }
            : handlePost}
          disabled={busy || (step === 'clips' && !clips.length)}
        ...
        >
          {uploading ? `${uploadProgress}%` : step === 'clips' ? 'Next' : 'Post'}
        </button>
      </div>
        ...
        {step === 'clips' ? (
          <div className="px-5 pt-4">
            <p className="text-[#6b7280] text-xs mb-4">
              Upload any length video — we'll auto-split it into 2-min clips for the slideshow.
            </p>

            <div className="grid grid-cols-3 gap-2 mb-4">
              {clips.map((clip, i) => (
                <div key={i} className="relative aspect-[9/16] rounded-xl overflow-hidden bg-[#1e2d4a]">
                  <video src={clip.url} className="w-full h-full object-cover" />
                  <div className="absolute top-1.5 left-1.5 w-5 h-5 rounded-full bg-black/70 flex items-center justify-center">
                    <span className="text-[#C9A84C] text-[9px] font-black">{i + 1}</span>
                  </div>
                  ...
                  {i > 0 && (
                    <button onClick={() => moveClip(i, i - 1)} disabled={busy}
                  ...
                  <button onClick={() => removeClip(i)} disabled={busy}
              ...
              {clips.length < MAX_CLIPS && !processing && (
                ...
                >
                  ...
                  <span className="text-[#6b7280] text-[10px]">Add video</span>
                </button>
              )}
            ...
            <p className="text-[#6b7280] text-[10px] text-center">
              Long videos auto-split into 2-min clips • Up to {MAX_CLIPS} clips
            </p>
          </div>
        ) : (
